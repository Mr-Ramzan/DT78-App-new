...
//        mChart.build()
    ...
    private fun updateDonut(context: Context, current: Int, target: Int, update: Boolean) {
        ...
        donut?.donutColors =
            intArrayOf(ContextCompat.getColor(requireContext(), R.color.primaryColor))
        if (update) {
...
//                        }
    ...
    private fun appsList() {
        ...
        val enabled =
            NotificationManagerCompat.getEnabledListenerPackages(requireContext()).contains(
...
//            title = Watch(tp).name
        checkEnabled(tp)
        ForegroundService.lst_sync =
            pref.getLong(SettingsActivity.PREF_SYNC, System.currentTimeMillis() - 604800000)
...
//            setCamera(pref.getBoolean(SettingsActivity.PREF_CAMERA, false), RootUtil.isDeviceRooted)
        ...
        updateDash(
            dbHandler.getHeartToday(),
            dbHandler.getBpToday()[0],
            dbHandler.getBpToday()[1],
            dbHandler.getSp02Today()
        )

        step?.text = stepsCal.steps.toString()
        cal?.text = "${stepsCal.calories} " + this.resources.getString(R.string.kcal)
        dis?.text =
            distance(stepsCal.steps * stepSize, ForegroundService.unit != 0, requireContext())
...
//        sleepList.add(SleepData(21, 2, 11, 5, 5, 2, 354))
        ...
        mBinding.textSleep.text = if (sleepList.isNotEmpty()) {
            ...
            mBinding.sleepDonut.donutColors = intArrayOf(
                requireContext().getColorFromAttr(R.attr.colorButtonEnabled),
                requireContext().getColorFromAttr(R.attr.colorIcons)
            )
            mBinding.sleepDonut.animate(
                arrayListOf(
                    ((lightS.toFloat() / (lightS + deepS)) * 100),
                    ((deepS.toFloat() / (lightS + deepS)) * 100)
                )
            )
        ...
        mBinding.quietActive.visibility =
            if (isQuietA(dbHandler.getSet(2))) View.VISIBLE else View.GONE
...
//        }
    ...
    private fun updateDash(hrm: Int, bpH: Int, bpL: Int, sp02: Int) {
        ...
        mBinding.hrmDonut.donutColors =
            intArrayOf(requireContext().getColorFromAttr(R.attr.colorIcons))
        mBinding.hrmDonut.animate(arrayListOf(map(hrm, 40, 100).toFloat()))
        mBinding.bpDonut.donutColors = intArrayOf(
            requireContext().getColorFromAttr(R.attr.colorIcons),
            requireContext().getColorFromAttr(R.attr.colorButtonEnabled)
        )
        mBinding.bpDonut.animate(
            arrayListOf(
                (bpH.toFloat() / (bpH + bpL) * 100),
                (bpL.toFloat() / (bpH + bpL) * 100)
            )
        )
        mBinding.spDonut.donutColors =
            intArrayOf(requireContext().getColorFromAttr(R.attr.colorIcons))
...
//    }
    ...
    private fun checkEnabled(id: Int) {
        if (id == ESP32) {
            ...
            mBinding.layoutSteps.backgroundTintList =
                ColorStateList.valueOf(requireContext().getColorFromAttr(R.attr.colorCardBackgroundDark))
            mBinding.linearLayout.backgroundTintList =
                ColorStateList.valueOf(requireContext().getColorFromAttr(R.attr.colorCardBackgroundDark))
            mBinding.reminder.backgroundTintList =
                ColorStateList.valueOf(requireContext().getColorFromAttr(R.attr.colorCardBackgroundDark))
            ...
            mBinding.reminder.setCardBackgroundColor(
        ...
        if (data.size() == 8) {
            if (data.getByte(4) == (0x91).toByte()) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    per?.imageTintList =
                        ColorStateList.valueOf(color(ForegroundService.bat, context))
                }
            }
        }
        ...
        when (view?.id) {
            R.id.cardInfo -> {
                if (ForegroundService().syncData()) {
                    ...
                    Toast.makeText(requireContext(), R.string.not_connect, Toast.LENGTH_SHORT)
                        .show()
                }
            }
            R.id.hrmButton -> {
    ...
    private fun setIcon(state: Boolean) {
        if (state) {
            ...
            bt?.imageTintList = ColorStateList.valueOf(
                ContextCompat.getColor(
                    requireContext(),
                    R.color.colorBluetooth
                )
            )
        } else {
